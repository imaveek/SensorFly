...
void TimerOne::setPwmDuty(char pin, int duty)
{
	...
	else if(pin == 10) OCR1B = dutyCycle;
	SREG = oldSREG;
}
...
void TimerOne::pwm(char pin, int duty, long microseconds)  // expects duty cycle to be 10 bit (1024)
{
	...
	if(pin == 11) {
		DDRB |= _BV(PORTB5);                                   // sets data direction register for pwm output pin
...
void TimerOne::disablePwm(char pin)
{
	...
	else if(pin == 10) TCCR1A &= ~_BV(COM1B1);   // clear the bit that enables pwm on PB6
}
